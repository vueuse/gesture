 ...
 */
export default abstract class DistanceAngleRecognizer<
...
> extends Recognizer<T> {
  ...
  getKinematics(values: Vector2, event: UIEvent): PartialGestureState<T> {
    ...
    const turns =
      (values[1] - state._movement![1] - this.state.initial[1]) / 360
  ...
  protected mapStateValues(
    ...
    return { da: state.values, vdva: state.velocities } as Omit<
