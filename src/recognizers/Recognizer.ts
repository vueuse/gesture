...
import { addV, sign, subV } from '../utils/math'
...
export default abstract class Recognizer<T extends StateKey = StateKey> {
  ...
  protected getMovement(values: Vector2): PartialGestureState<T> {
    ...
    const i0 =
      wasIntentional[0] === false
        ? getIntentionalDisplacement(M[0], _T[0])
        : wasIntentional[0]
    const i1 =
      wasIntentional[1] === false
  ...
  protected fireGestureHandler = (
    ...
    const touches =
      this.controller.pointerIds.size || this.controller.touchIds.size
...
function getIntentionalDisplacement(
  movement: number,
  threshold: number,
): number | false {
