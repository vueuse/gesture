...
> extends Recognizer<T> {
  ...
  protected checkIntentionality(
    ...
    const axis =
      this.state.axis || (absX > absY ? 'x' : absX < absY ? 'y' : undefined)
    if (!this.config.axis && !this.config.lockDirection)
      return { _intentional, _blocked: false, axis } as any
    if (!axis)
      return { _intentional: [false, false], _blocked: false, axis } as any
    if (!!this.config.axis && axis !== this.config.axis)
      return { _intentional, _blocked: true, axis } as any
  ...
  protected mapStateValues(
    state: GestureState<T>,
  ): Omit<PartialGestureState<T>, 'event'> {
    return { xy: state.values, vxvy: state.velocities } as Omit<
      PartialGestureState<T>,
      'event'
    >
  }
