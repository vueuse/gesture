...
import { assignDefault, ensureVector, valueFn } from './utils'
...
const InternalCoordinatesOptionsNormalizers = {
  ...
  bounds(value: Bounds | ((state?: State) => Bounds) = {}) {
    if (typeof value === 'function')
      return (state?: State) =>
        InternalCoordinatesOptionsNormalizers.bounds(value(state))
...
const InternalDistanceAngleOptionsNormalizers = {
  ...
  bounds(
    ...
    if (
      typeof distanceBounds !== 'function' &&
...
export function getInternalGestureOptions<T extends StateKey>(
  ...
  return resolveWith<GestureOptions<T>, InternalGestureOptions<T>>(
    config,
    InternalGestureOptionsNormalizers,
  )
}
...
export function getInternalCoordinatesOptions<T extends CoordinatesKey>(
  ...
  return resolveWith<CoordinatesConfig<T>, InternalCoordinatesOptions<T>>(
    config,
    InternalCoordinatesOptionsNormalizers,
  )
}
...
export function getInternalDragOptions(
  config: DragConfig = {},
): InternalDragOptions {
  return resolveWith<DragConfig, InternalDragOptions>(
    config,
